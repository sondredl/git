                          ...
                          int                show_only)
{
	...
	for (size_t i = 0; i < repo->index->cache_nr; i++) {
        ...
        if (!include_sparse && (ce_skip_worktree(ce) || !path_in_sparse_checkout(ce->name, repo->index)))
            continue;
...
static int refresh(struct repository *repo, int verbose, const struct pathspec *pathspec)
{
    ...
    for (i = 0; i < pathspec->nr; i++)
    {
        if (!seen[i])
        {
            ...
            if (matches_skip_worktree(pathspec, i, &skip_worktree_seen) || !path_in_sparse_checkout(path, repo->index))
            {
...
static int add_files(struct repository *repo, struct dir_struct *dir, int flags)
{
    ...
    for (i = 0; i < dir->nr; i++)
    {
        if (!include_sparse && !path_in_sparse_checkout(dir->entries[i]->name, repo->index))
        {
