                            ...
                            struct test_entry *entry)
{
    ...
    {
        if (!strcmp(entry->key, key_val[i][0]) && !strcmp(get_value(entry), key_val[i][1]))
        {
            if (seen[i])
                return 2;
            seen[i] = 1;
...
static void t_add(struct hashmap *map, unsigned int ignore_case)
{
    ...
    const char *query_keys[]              = {"key1", ignore_case ? "FOObarFrotz" : "fooBarFrotz"};
    ...
    for (size_t i = 0; i < ARRAY_SIZE(query_keys); i++)
    {
        ...
        entry     = hashmap_get_entry_from_hash(map,
                                            ignore_case ? strihash(query_keys[i]) : strhash(query_keys[i]),
    ...
    for (size_t i = 0; i < ARRAY_SIZE(seen); i++)
    {
        if (!check_int(seen[i], ==, 1))
            test_msg(
                "following key-val pair was not iterated over:\n"
                "    key: %s\n  value: %s",
