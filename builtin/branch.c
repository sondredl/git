                         ...
                         struct commit *rev, struct commit *head_rev)
{
  ...
  if (kind == FILTER_REFS_BRANCHES)
  {
    ...
    if (upstream && (reference_name = reference_name_to_free = resolve_refdup(upstream, RESOLVE_REF_READING, &oid, NULL)) != NULL)
                           ...
                           int quiet)
{
  ...
  for (i = 0; i < argc; i++, strbuf_reset(&bname))
  {
    ...
    if (!(flags & (REF_ISSYMREF | REF_ISBROKEN)) && check_branch_commit(bname.buf, name, &oid, head_rev, kinds, force))
    {
      ret = 1;
      goto next;
    }
...
static void copy_or_rename_branch(const char *oldname, const char *newname, int copy, int force)
{
  ...
  if (!skip_prefix(oldref.buf, "refs/heads/", &interpreted_oldname) || !skip_prefix(newref.buf, "refs/heads/", &interpreted_newname))
  {
  ...
  if (!copy && !(oldref_usage & IS_ORPHAN) && rename_ref(oldref.buf, newref.buf, logmsg.buf))
...
int cmd_branch(int argc, const char **argv, const char *prefix)
{
  ...
  else if (unset_upstream)
  {
    ...
    if (!branch)
    {
      if (!argc || !strcmp(argv[0], "HEAD"))
        die(_(
            "could not unset upstream of HEAD when "
            "it does not point to any branch"));
      die(_("no such branch '%s'"), argv[0]);
    }
