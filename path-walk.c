                            ...
                            struct object_id         *oid)
{
    ...
    while (tree_entry(&desc, &entry))
    {
        ...
        if (ctx->info->pl)
        {
            ...
            if (ctx->info->pl->use_cone_patterns && match == NOT_MATCHED)
                continue;
            else if (!ctx->info->pl->use_cone_patterns && type == OBJ_BLOB && match != MATCHED)
                continue;
        }
                     ...
                     const char               *path)
{
    ...
    if ((list->type == OBJ_TREE && ctx->info->trees) || (list->type == OBJ_BLOB && ctx->info->blobs) || (list->type == OBJ_TAG && ctx->info->tags))
