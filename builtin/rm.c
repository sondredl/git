...
static int check_local_mod(struct object_id *head, int index_only)
{
  ...
  for (i = 0; i < list.nr; i++)
  {
    ...
    if (pos < 0)
    {
      ...
      if (!S_ISGITLINK(the_index.cache[pos]->ce_mode) || is_empty_dir(name))
        continue;
    }
    ce = the_index.cache[pos];
     ...
     */
    if (ie_match_stat(&the_index, ce, &st, 0) || (S_ISGITLINK(ce->ce_mode) && bad_to_remove_submodule(ce->name, SUBMODULE_REMOVAL_DIE_ON_ERROR | SUBMODULE_REMOVAL_IGNORE_IGNORED_UNTRACKED)))
      local_changes = 1;
...
int cmd_rm(int argc, const char **argv, const char *prefix)
{
  ...
  for (i = 0; i < the_index.cache_nr; i++)
  {
    ...
    if (!include_sparse && (ce_skip_worktree(ce) || !path_in_sparse_checkout(ce->name, &the_index)))
      continue;
    ...
    if (list.entry[list.nr++].is_submodule && !is_staging_gitmodules_ok(&the_index))
