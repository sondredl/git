...
static double clar_time_diff(clar_time *start, clar_time *end)
{
    return ((double)end->tv_sec + (double)end->tv_usec / 1.0E6) - ((double)start->tv_sec + (double)start->tv_usec / 1.0E6);
}
#endif
            ...
            { /* given suite name */
                ...
                for (j = 0; j < _clar_suite_count; ++j)
                {
                    ...
                    if (strncmp(argument, _clar_suites[j].name, cmplen) == 0)
                    {
                        ...
                        if (arglen > suitelen && (argument[suitelen] != ':' || argument[suitelen + 1] != ':'))
                            continue;
...
void clar_test_init(int argc, char **argv)
{
    ...
    if (!_clar.summary_filename && (summary_env = getenv("CLAR_SUMMARY")) != NULL)
    {
        _clar.write_summary    = 1;
