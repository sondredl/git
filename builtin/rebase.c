...
#include "components/rebase-interactive.h"
...
int cmd_rebase(int argc, const char **argv, const char *prefix)
{
  ...
  if (allow_preemptive_ff && can_fast_forward(options.onto, options.upstream, options.restrict_revision, options.orig_head, &branch_base))
  {
    ...
    if (!(options.flags & REBASE_FORCE))
    {
      ...
      if (!(options.flags & REBASE_NO_QUIET))
        ; /* be quiet */
      else if (!strcmp(branch_name, "HEAD") && resolve_ref_unsafe("HEAD", 0, NULL, &flag))
  ...
  if (!ok_to_skip_pre_rebase && run_hooks_l("pre-rebase", options.upstream_arg, argc ? argv[0] : NULL, NULL))
