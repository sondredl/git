...
{
    if (get_oidf(&info->b_commit, "%s^1", revision) || get_oidf(&info->w_tree, "%s:", revision) || get_oidf(&info->b_tree, "%s^1:", revision) || get_oidf(&info->i_tree, "%s^2:", revision))
    {
		       ...
		       struct repository *repo UNUSED)
{
    ...
    {
        return error(_(
            "git stash clear with arguments is "
            "unimplemented"));
    }
		     ...
		     struct repository *repo UNUSED)
{
    ...
    {
        printf_ln(_(
            "The stash entry is kept in case "
            "you need it again."));
    }
    else
    {
                         ...
                         int keep_index, int patch_mode, int include_untracked, int only_staged)
{
    ...
    if (!(patch_mode || only_staged))
    {
         ...
         */
        if (keep_index == 1 && !is_null_oid(&info.i_tree) && !is_empty_tree_oid(&info.i_tree, the_repository->hash_algo))
        {
            struct child_process cp = CHILD_PROCESS_INIT;
                      ...
                      int push_assumed)
{
    ...
    if (argc)
    {
        ...
        argc         = parse_options(argc, argv, prefix, options,
                             push_assumed ? git_stash_usage : git_stash_push_usage,
	      ...
	      struct repository *repo)
{
    ...
    argc = parse_options(argc, argv, prefix, options, git_stash_usage,
                         PARSE_OPT_SUBCOMMAND_OPTIONAL | PARSE_OPT_KEEP_UNKNOWN_OPT | PARSE_OPT_KEEP_DASHDASH);
