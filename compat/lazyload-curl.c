    ...
    if (len && len + name_size < sizeof(dll_path))
    {
      ...
      if (!access(dll_path, R_OK))
      {
        ...
        if (!res)
        {
          ...
          if (!FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_ARGUMENT_ARRAY | FORMAT_MESSAGE_IGNORE_INSERTS,
...
CURLcode curl_easy_getinfo(CURL *curl, CURLINFO info, ...)
{
  ...
  CURL_IGNORE_DEPRECATION(
      ...
      else if ((info >= CURLINFO_STRING && info < CURLINFO_LONG) || (info >= CURLINFO_SLIST && info < CURLINFO_SOCKET))
