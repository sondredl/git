...
static void detect_msys_tty(int fd)
{
  ...
  if (!NT_SUCCESS(NtQueryObject(h, ObjectNameInformation,
    ...
    return;
  if (result < sizeof(*nameinfo) || !nameinfo->Name.Buffer || !nameinfo->Name.Length)
    return;
...
static int enable_virtual_processing(void)
{
  ...
  if (std_console_handle == INVALID_HANDLE_VALUE || !GetConsoleMode(std_console_handle, &std_console_mode))
  {
  ...
  if (!SetConsoleMode(std_console_handle,
                      std_console_mode | ENABLE_PROCESSED_OUTPUT | ENABLE_VIRTUAL_TERMINAL_PROCESSING))
    return 0;
