...
{
    int len = strlen(dir);
    return !strncmp(buf, dir, len) && (is_dir_sep(buf[len]) || buf[len] == '\0');
}
...
{
    int len = strlen(dir);
    if (strncmp(buf, dir, len) != 0 || !is_dir_sep(buf[len]))
    {
        return 0;
    }
...
static void replace_dir(struct strbuf *buf, int len, const char *newdir)
{
    ...
    int need_sep = (buf->buf[len] && !is_dir_sep(buf->buf[len])) && !is_dir_sep(newdir[newlen - 1]);
    if (need_sep)
    {
...
const char *enter_repo(const char *path, int strict)
{
    ...
    if (!strict)
    {
        ...
        for (i = 0; suffix[i]; i++)
        {
            ...
            if (!stat(used_path.buf, &st) && (S_ISREG(st.st_mode) || (S_ISDIR(st.st_mode) && is_git_directory(used_path.buf))))
            {
...
int adjust_shared_perm(const char *path)
{
    ...
    if (((old_mode ^ new_mode) & ~S_IFMT) && chmod(path, (new_mode & ~S_IFMT)) < 0)
    {
        return -2;
    }
    return 0;
...
static int have_same_root(const char *path1, const char *path2)
{
    ...
    return (is_abs1 && is_abs2 && tolower(path1[0]) == tolower(path2[0])) || (!is_abs1 && !is_abs2);
}
...
int longest_ancestor_length(const char *path, struct string_list *prefixes)
{
    ...
    for (i = 0; i < prefixes->nr; i++)
    {
        ...
        if (strncmp(path, ceil, len) != 0 || path[len] != '/' || !path[len + 1])
        {
            continue; /* no match */
        }
...
int is_ntfs_dotgit(const char *name)
{
    ...
    {
        /* .git */
        if (((c = *(name++)) != 'g' && c != 'G') || ((c = *(name++)) != 'i' && c != 'I') || ((c = *(name++)) != 't' && c != 'T'))
        {
            return 0;
        }
    }
    ...
    {
        /* git ~1 */
        if (((c = *(name++)) != 'i' && c != 'I') || ((c = *(name++)) != 't' && c != 'T') || *(name++) != '~' || *(name++) != '1')
        {
            return 0;
        }
    }
                               ...
                               const char *dotgit_ntfs_shortname_prefix)
{
     ...
     * followed by ~1, ... ~4?
     */
    if (!strncasecmp(name, dotgit_name, 6) && name[6] == '~' && name[7] >= '1' && name[7] <= '4')
    {
        i = 8;
