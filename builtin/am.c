 ...
 * at the end.
 */
__attribute__((format(printf, 3, 4))) static void say(const struct am_state *state, FILE *fp, const char *fmt, ...)
{
    va_list ap;
...
static void am_load(struct am_state *state)
{
    ...
    if (file_exists(am_path(state, "rerere-autoupdate")))
    {
        ...
        state->allow_rerere_autoupdate = strcmp(sb.buf, "t") ? RERERE_NOAUTOUPDATE : RERERE_AUTOUPDATE;
    }
    else
    {
...
static int detect_patch_format(const char **paths)
{
     ...
     */
    if (l1.len && !l2.len && (starts_with(l3.buf, "From:") || starts_with(l3.buf, "Author:") || starts_with(l3.buf, "Date:")))
    {
...
static int stgit_patch_to_mail(FILE *out, FILE *in, int keep_cr UNUSED)
{
    ...
    while (!strbuf_getline_lf(&sb, in))
    {
        ...
        if (skip_prefix(sb.buf, "Author:", &str))
            fprintf(out, "From:%s\n", str);
...
static void NORETURN die_user_resolve(const struct am_state *state)
{
    ...
    else
    {
        ...
        if (advice_enabled(ADVICE_AM_WORK_DIR) && is_empty_or_missing_file(am_path(state, "patch")) && !repo_index_has_changes(the_repository, NULL, NULL))
        {
...
static int get_mail_commit_oid(struct object_id *commit_id, const char *mail)
{
    ...
    if (strbuf_getline_lf(&sb, fp) || !skip_prefix(sb.buf, "From ", &x) || get_oid_hex(x, commit_id) < 0)
    {
        ret = -1;
    }
...
static void am_run(struct am_state *state, int resume)
{
    ...
    while (state->cur <= state->last)
    {
        ...
        if (apply_status && state->threeway)
        {
             ...
             */
            if (!apply_status && !repo_index_has_changes(the_repository, NULL, NULL))
            {
...
static int clean_index(const struct object_id *head, const struct object_id *remote)
{
    ...
    struct tree     *remote_tree;
    struct tree     *index_tree;
