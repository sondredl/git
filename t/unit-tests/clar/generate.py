...
class Module(object):
    ...
    class DeclarationTemplate(Template):
        def render(self):
            out = "\n".join(
                "extern %s;" %
                cb['declaration'] for cb in self.module.callbacks) + "\n"
    ...
    class CallbacksTemplate(Template):
        def render(self):
            ...
            out += ",\n".join(self._render_callback(cb)
                              for cb in self.module.callbacks)
            out += "\n};\n"
            return out
    ...
    class InfoTemplate(Template):
        def render(self):
            ...
            for initializer in initializers:
                ...
                if initializer and initializer['short_name'].startswith(
                        'initialize_'):
...
class TestSuite(object):
    ...
    def find_modules(self):
        ...
        for root, _, files in os.walk(self.path):
            ...
            for test_file in tests_in_module:
                ...
                module_name = "_".join(
                    module_root + [test_file[:-2]]).replace("-", "_")
    ...
    def write(self):
        ...
        with open(output, 'w') as data:
            ...
            data.write(
                "static const size_t _clar_suite_count = %d;\n" %
                self.suite_count())
            data.write(
                "static const size_t _clar_callback_count = %d;\n" %
                self.callback_count())
