...
static char *get_value(char *buf, const char *key)
{
    ...
    if (!buf || !skip_prefix((const char *)buf, key, (const char **)&buf) || !skip_prefix((const char *)buf, "=", (const char **)&buf) || !*buf)
...
static void command_loop(void)
{
    for (;;)
    {
        ...
        while ((buf = packet_read_line(0, NULL)))
        {
            ...
            else if (starts_with(buf, "ref=") || starts_with(buf, "treeish=") || starts_with(buf, "blob="))
            {
        ...
        else if (!strcmp(command, "smudge") && (entry = strmap_get(&delay, pathname)) && entry->requested == 1)
        {
        ...
        else
        {
            ...
            if (skip_prefix(pathname, command, &p) && !strcmp(p, "-write-fail.r"))
            {
