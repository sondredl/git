...
#define OPT_BIT_F(s, l, v, h, b, f)                                        \
	{                                                                  \
		...
		.value = (v), .help = (h), .flags = PARSE_OPT_NOARG | (f), \
		.callback = NULL, .defval = (b),                           \
	}
#define OPT_COUNTUP_F(s, l, v, h, f)                                         \
	{                                                                    \
		.type = OPTION_COUNTUP, .short_name = (s), .long_name = (l), \
		.value = (v), .help = (h), .flags = PARSE_OPT_NOARG | (f),   \
	}
#define OPT_SET_INT_F(s, l, v, h, i, f)                                      \
	{                                                                    \
		.type = OPTION_SET_INT, .short_name = (s), .long_name = (l), \
		.value = (v), .help = (h), .flags = PARSE_OPT_NOARG | (f),   \
		.defval = (i),                                               \
	}
...
#define OPT_INTEGER_F(s, l, v, h, f)                                         \
	{                                                                    \
		...
		.value = (v), .argh = N_("n"), .help = (h), .flags = (f),    \
	}
...
#define OPT_BITOP(s, l, v, h, set, clear)                                    \
	{                                                                    \
		.type = OPTION_BITOP, .short_name = (s), .long_name = (l),   \
		.value = (v), .help = (h),                                   \
		.flags = PARSE_OPT_NOARG | PARSE_OPT_NONEG, .defval = (set), \
		.extra = (clear),                                            \
	}
#define OPT_NEGBIT(s, l, v, h, b)                                           \
	{                                                                   \
		.type = OPTION_NEGBIT, .short_name = (s), .long_name = (l), \
		.value = (v), .help = (h), .flags = PARSE_OPT_NOARG,        \
		.defval = (b),                                              \
	}
...
#define OPT_HIDDEN_BOOL(s, l, v, h)                                          \
	{                                                                    \
		.type = OPTION_SET_INT, .short_name = (s), .long_name = (l), \
		.value = (v), .help = (h),                                   \
		.flags = PARSE_OPT_NOARG | PARSE_OPT_HIDDEN, .defval = 1,    \
	}
#define OPT_CMDMODE_F(s, l, v, h, i, f)                                      \
	{                                                                    \
		.type = OPTION_SET_INT, .short_name = (s), .long_name = (l), \
		.value = (v), .help = (h),                                   \
		.flags = PARSE_OPT_CMDMODE | PARSE_OPT_NOARG |               \
			 PARSE_OPT_NONEG | (f),                              \
		.defval = (i),                                               \
	}
...
#define OPT_MAGNITUDE(s, l, v, h)                                              \
	{                                                                      \
		.type = OPTION_MAGNITUDE, .short_name = (s), .long_name = (l), \
		.value = (v), .argh = N_("n"), .help = (h),                    \
...
#define OPT_EXPIRY_DATE(s, l, v, h)                                           \
	{                                                                     \
		.type = OPTION_CALLBACK, .short_name = (s), .long_name = (l), \
		.value = (v), .argh = N_("expiry-date"), .help = (h),         \
...
#define OPT_FILENAME(s, l, v, h)                                              \
	{                                                                     \
		...
		.value = (v), .argh = N_("file"), .help = (h),                \
	}
#define OPT_COLOR_FLAG(s, l, v, h)                                            \
	{                                                                     \
		.type = OPTION_CALLBACK, .short_name = (s), .long_name = (l), \
		.value = (v), .argh = N_("when"), .help = (h),                \
...
#define OPT_SUBCOMMAND_F(l, v, fn, f)                                      \
	{                                                                  \
		.type = OPTION_SUBCOMMAND, .long_name = (l), .value = (v), \
		.flags = (f), .subcommand_fn = (fn),                       \
	}
...
#define OPT__VERBOSITY(var)                                            \
	...
	{                                                              \
		.type = OPTION_CALLBACK, .short_name = 'q',            \
		.long_name = "quiet", .value = (var),                  \
		.help = N_("be more quiet"), .flags = PARSE_OPT_NOARG, \
		.callback = &parse_opt_verbosity_cb,                   \
	}
...
#define OPT__ABBREV(var)                                                     \
	{                                                                    \
		.type = OPTION_CALLBACK, .long_name = "abbrev",              \
		.value = (var), .argh = N_("n"),                             \
...
#define OPT__SUPER_PREFIX(var)                                    \
	OPT_STRING_F(0, "super-prefix", (var), N_("prefix"),      \
		     N_("prefixed path to initial superproject"), \
		     PARSE_OPT_HIDDEN)
