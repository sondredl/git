                            ...
                            struct ipc_server_reply_data  *reply_data)
{
    ...
    else if (!skip_prefix(command, "builtin:", &p))
    {
        ...
        trace_printf_key(&trace_fsmonitor,
                         ((*p_end) ? "fsmonitor: invalid command line '%s'" : "fsmonitor: unsupported V1 protocol '%s'"),
                         command);
        do_trivial = 1;
        do_cookie  = 1;
    }
    ...
    if (!do_trivial)
    {
        ...
        else if (requested_oldest_seq_nr < token_data->batch_tail->batch_seq_nr)
        {
            /*
...
static void *fsm_listen__thread_proc(void *_state)
{
    ...
    pthread_mutex_lock(&state->main_lock);
    if (state->current_token_data && state->current_token_data->client_ref_count == 0)
