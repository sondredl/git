                                   ...
                                   struct base_data    *parent)
{
    ...
    base->children_remaining = base->ref_last - base->ref_first + base->ofs_last - base->ofs_first + 2;
    return base;
}
...
static int compare_ofs_delta_entry(const void *a, const void *b)
{
    ...
    return delta_a->offset < delta_b->offset ? -1 : delta_a->offset > delta_b->offset ? 1
                                                                                      : 0;
}
...
static void *threaded_second_pass(void *data)
{
    ...
    for (;;)
    {
        ...
        else
        {
            ...
            {
                child_obj = objects + ofs_deltas[parent->ofs_first++].obj_no;
                assert(child_obj->real_type == OBJ_OFS_DELTA);
                child_obj->real_type = parent->obj->real_type;
            }

            if (parent->ref_first > parent->ref_last && parent->ofs_first > parent->ofs_last)
            {
                /*
