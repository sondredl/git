                                           ...
                                           const char       *dirname)
{
  ...
  while ((de = readdir(d)) != NULL)
  {
    ...
    dtype = get_dtype(de, &path, 1);
    if (dtype == DT_REG && (is_pseudoref(ref_store, de->d_name) || is_headref(ref_store, de->d_name)))
      loose_fill_ref_dir_regular_file(refs, refname.buf, dir);
                                    ...
                                    const char *logmsg, int copy)
{
  ...
  if (write_ref_to_lockfile(lock, &orig_oid, 0, &err) || commit_ref_update(refs, lock, &orig_oid, logmsg, &err))
  {
...
rollback:
  ...
  if (write_ref_to_lockfile(lock, &orig_oid, 0, &err) || commit_ref_update(refs, lock, &orig_oid, NULL, &err))
  {
                                 ...
                                 int skip_oid_verification, struct strbuf *err)
{
  ...
  if (write_in_full(fd, oid_to_hex(oid), the_hash_algo->hexsz) < 0 || write_in_full(fd, &term, 1) < 0 || fsync_component(FSYNC_COMPONENT_REFERENCE, get_lock_file_fd(&lock->lk)) < 0 || close_ref_gently(lock) < 0)
  {
    strbuf_addf(err,
                                 ...
                                 const char *target, const char *logmsg)
{
  ...
  if (logmsg && refs_resolve_ref_unsafe(&refs->base, target, RESOLVE_REF_READING, &new_oid, NULL) && files_log_ref_write(refs, refname, &lock->old_oid, &new_oid, logmsg, 0, &err))
  {
    error("%s", err.buf);
    strbuf_release(&err);
  }
                             ...
                             struct strbuf          *err)
{
  ...
  if ((update->flags & REF_LOG_ONLY) || (update->flags & REF_IS_PRUNING) || (update->flags & REF_UPDATE_VIA_HEAD))
    return 0;
                               ...
                               struct strbuf          *err)
{
  ...
  if ((update->flags & REF_HAVE_NEW) && !(update->flags & REF_DELETING) && !(update->flags & REF_LOG_ONLY))
  {
                               ...
                               void                         *policy_cb_data)
{
  ...
  if (!cb.dry_run)
  {
    ...
    else if (update && (write_in_full(get_lock_file_fd(&lock->lk), oid_to_hex(&cb.last_kept_oid), the_hash_algo->hexsz) < 0 || write_str_in_full(get_lock_file_fd(&lock->lk), "\n") < 0 || close_ref_gently(lock) < 0))
    {
