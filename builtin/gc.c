...
static void gc_config(struct gc_config *cfg)
{
    ...
    if (gc_config_is_timestamp_never("gc.reflogexpire") && gc_config_is_timestamp_never("gc.reflogexpireunreachable"))
        cfg->prune_reflogs = 0;
...
static int too_many_loose_objects(struct gc_config *cfg)
{
    ...
    {
        if (strspn(ent->d_name, "0123456789abcdef") != hexsz_loose || ent->d_name[hexsz_loose] != '\0')
            continue;
...
static const char *lock_repo_for_gc(int force, pid_t *ret_pid)
{
    ...
    if (!force)
    {
        ...
        should_exit =
             ...
             * running.
             */
            time(NULL) - st.st_mtime <= 12 * 3600 && fscanf(fp, scan_fmt, &pid, locking_host) == 2 &&
            /* be gentle to concurrent "gc" on remote hosts */
            (strcmp(locking_host, my_host) != 0 || !kill(pid, 0) || errno == EPERM);
        if (fp)
        {
            fclose(fp);
        }
                      ...
                      void                   *cb_data)
{
    ...
    if (repo_parse_commit(the_repository, commit) || commit_graph_position(commit) != COMMIT_NOT_FROM_GRAPH)
    {
        return 0;
    }
    ...
    while (!result && stack)
    {
        ...
        {
            if (repo_parse_commit(the_repository, parent->item) || commit_graph_position(parent->item) != COMMIT_NOT_FROM_GRAPH || parent->item->object.flags & SEEN)
            {
                continue;
            }
                                 ...
                                 struct gc_config            *cfg)
{
    ...
    for (i = 0; i < TASK__COUNT; i++)
    {
        ...
        if (opts->auto_flag && (!tasks[i].auto_condition || !tasks[i].auto_condition(cfg)))
            continue;
...
{
    struct strbuf sb = STRBUF_INIT;
    const char   *p  = the_repository->worktree ? the_repository->worktree : the_repository->gitdir;
...
{
    return launchctl_remove_plist(SCHEDULE_HOURLY) || launchctl_remove_plist(SCHEDULE_DAILY) || launchctl_remove_plist(SCHEDULE_WEEKLY);
}
...
static int launchctl_schedule_plist(const char *exec_path, enum schedule_priority schedule)
{
    ...
    if ((long)lock_file_timeout_ms < 0 && git_config_get_ulong("gc.launchctlplistlocktimeoutms", &lock_file_timeout_ms))
    {
     ...
     */
    if (!stat(filename, &st) && st.st_size == plist.len && strbuf_read_file(&plist2, filename, plist.len) == plist.len && !strbuf_cmp(&plist, &plist2) && launchctl_list_contains_plist(name, cmd))
    {
        rollback_lock_file(&lk);
    }
    else
    {
        if (write_in_full(fd, plist.buf, plist.len) < 0 || commit_lock_file(&lk))
        {
...
static int launchctl_add_plists(void)
{
    ...
    return launchctl_schedule_plist(exec_path, SCHEDULE_HOURLY) || launchctl_schedule_plist(exec_path, SCHEDULE_DAILY) || launchctl_schedule_plist(exec_path, SCHEDULE_WEEKLY);
}
...
{
    return schtasks_remove_task(SCHEDULE_HOURLY) || schtasks_remove_task(SCHEDULE_DAILY) || schtasks_remove_task(SCHEDULE_WEEKLY);
}
...
static int schtasks_schedule_tasks(void)
{
    ...
    return schtasks_schedule_task(exec_path, SCHEDULE_HOURLY) || schtasks_schedule_task(exec_path, SCHEDULE_DAILY) || schtasks_schedule_task(exec_path, SCHEDULE_WEEKLY);
}
...
static int systemd_timer_delete_unit_files(void)
{
    ...
    return systemd_timer_delete_timer_file(SCHEDULE_HOURLY) | systemd_timer_delete_timer_file(SCHEDULE_DAILY) | systemd_timer_delete_timer_file(SCHEDULE_WEEKLY) | systemd_timer_delete_service_template();
}
...
static int systemd_timer_delete_units(void)
{
    int minute = get_random_minute();
    /* Purposefully not short-circuited to make sure all are called. */
    return systemd_timer_enable_unit(0, SCHEDULE_HOURLY, minute) | systemd_timer_enable_unit(0, SCHEDULE_DAILY, minute) | systemd_timer_enable_unit(0, SCHEDULE_WEEKLY, minute) | systemd_timer_delete_unit_files();
}
...
static int systemd_timer_setup_units(void)
{
    ...
    int ret = systemd_timer_write_service_template(exec_path) || systemd_timer_enable_unit(1, SCHEDULE_HOURLY, minute) || systemd_timer_enable_unit(1, SCHEDULE_DAILY, minute) || systemd_timer_enable_unit(1, SCHEDULE_WEEKLY, minute);
...
static enum scheduler parse_scheduler(const char *value)
{
    ...
    else if (!strcasecmp(value, "systemd") || !strcasecmp(value, "systemd-timer"))
