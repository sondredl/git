                                        ...
                                        xpparam_t const *xpp)
{
  ...
  for (; m; m = m->next)
  {
    ...
    while (m->chg1 && m->chg2 && recmatch(rec1[m->i1 + m->chg1 - 1], rec2[m->i2 + m->chg2 - 1], xpp->flags))
    {
      m->chg1--;
      m->chg2--;
    }
                                      ...
                                      int simplify_if_no_alnum)
{
  ...
  for (;;)
  {
    ...
    if (m->mode != 0 || next_m->mode != 0 || (end - begin > 3 && (!simplify_if_no_alnum || lines_contain_alnum(xe1, begin, end - begin))))
    {
      m = next_m;
    }
    else
                        ...
                        xmparam_t const *xmp, mmbuffer_t *result)
{
  ...
  while (xscr1 && xscr2)
  {
    ...
    if (level == XDL_MERGE_MINIMAL || xscr1->i1 != xscr2->i1 || xscr1->chg1 != xscr2->chg1 || xscr1->chg2 != xscr2->chg2 || xdl_merge_cmp_lines(xe1, xscr1->i2, xe2, xscr2->i2, xscr1->chg2, xpp->flags))
    {
      /* conflict */
  ...
  else if (XDL_MERGE_ZEALOUS <= level && (xdl_refine_conflicts(xe1, xe2, changes, xpp) < 0 || xdl_simplify_non_conflicts(xe1, changes, XDL_MERGE_ZEALOUS < level) < 0))
  {
    xdl_cleanup_merge(changes);
    return -1;
  }
              ...
              xmparam_t const *xmp, mmbuffer_t *result)
{
  ...
  if (xdl_change_compact(&xe1.xdf1, &xe1.xdf2, xpp->flags) < 0 || xdl_change_compact(&xe1.xdf2, &xe1.xdf1, xpp->flags) < 0 || xdl_build_script(&xe1, &xscr1) < 0)
    goto out;

  if (xdl_change_compact(&xe2.xdf1, &xe2.xdf2, xpp->flags) < 0 || xdl_change_compact(&xe2.xdf2, &xe2.xdf1, xpp->flags) < 0 || xdl_build_script(&xe2, &xscr2) < 0)
    goto out;
