...
int check_apply_state(struct apply_state *state, int force_apply)
{
	...
	if (state->cached) {
		if (is_not_gitdir)
			return error(_("'%s' outside a repository"),
				     "--cached");
		state->check_index = 1;
	}
...
static int gitdiff_hdrend(struct gitdiff_data *state UNUSED,
			  const char *line UNUSED, struct patch *patch UNUSED)
{
	return 1;
}
...
static int gitdiff_similarity(struct gitdiff_data *state UNUSED,
			      const char *line, struct patch *patch)
{
			    ...
			    unsigned mode)
{
	if (S_ISGITLINK(mode)) {
		...
		result = repo_read_object_file(the_repository, oid, &type, &sz);
		if (!result)
			return -1;
...
static int resolve_to(struct image *image, const struct object_id *result_id)
{
	...
	image->buf =
		repo_read_object_file(the_repository, result_id, &type, &size);
			...
			const struct cache_entry *ce)
{
	...
	else if (repo_get_oid(the_repository, patch->old_oid_prefix,
			      &pre_oid) ||
		...
		if (S_ISGITLINK(patch->old_mode)) {
			if (!preimage_oid_in_gitlink_patch(patch, &oid))
				...
				return error(_("sha1 information is lacking or "
					     ...
					     name);
		} else if (!repo_get_oid_blob(the_repository,
					      patch->old_oid_prefix, &oid)) {
			; /* ok */
			   ...
			   unsigned long size)
{
	...
	if (has_symlinks && S_ISLNK(mode))
		...
		return !!create_symlink(
			state && state->repo ? state->repo->index : NULL, buf,
			path);
...
static int write_out_results(struct apply_state *state, struct patch *list)
{
	...
	if (cpath.nr) {
		...
		/*
		 * rerere relies on the partially merged result being in the
		 * working tree with conflict markers, but that isn't written
		 * with --cached.
		 */
		if (!state->cached)
