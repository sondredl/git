...
int cmd_mv(int argc, const char **argv, const char *prefix)
{
  ...
  {
    if (!path_in_sparse_checkout(dst_w_slash, &the_index) && empty_dir_has_sparse_contents(dst_w_slash))
    {
      destination = internal_prefix_pathspec(dst_w_slash, argv, argc, DUP_BASENAME);
  ...
  for (i = 0; i < argc; i++)
  {
    ...
    if (lstat(src, &st) < 0)
    {
      ...
      {
        const char *src_w_slash = add_slash(src);
        if (!path_in_sparse_checkout(src_w_slash, &the_index) && empty_dir_has_sparse_contents(src))
        {
  ...
  dir_check:
    ...
    if (ignore_sparse && (dst_mode & (SKIP_WORKTREE_DIR | SPARSE)) && index_entry_exists(&the_index, dst, strlen(dst)))
    {
  ...
  for (i = 0; i < argc; i++)
  {
    ...
    if (ignore_sparse && core_apply_sparse_checkout && core_sparse_checkout_cone)
    {
       ...
       */
      if ((mode & SPARSE) && path_in_sparse_checkout(dst, &the_index))
      {
      ...
      else if ((dst_mode & (SKIP_WORKTREE_DIR | SPARSE)) && !(mode & SPARSE) && !path_in_sparse_checkout(dst, &the_index))
      {
