...
int are_long_paths_enabled(void)
{
  ...
  if (enabled < 0)
  {
    ...
    if (the_repository->config && the_repository->config->hash_initialized && git_config_get_bool("core.longpaths", &enabled) < 0)
      enabled = 0;
  }
                                       ...
                                       size_t size)
{
  ...
  if (is_wdir_sep(path[0]) || (iswalpha(path[0]) && path[1] == L':' && is_wdir_sep(path[2])))
    return path;
    ...
    process_phantom_symlink(const wchar_t *wtarget, const wchar_t *wlink)
{
  ...
  if (DeleteFileW(wlink) && CreateSymbolicLinkW(wlink, wtarget, symlink_directory_flags))
...
int mingw_unlink(const char *pathname)
{
  ...
  do
  {
    ...
    if (!_wrmdir(wpathname))
      return 0;
  } while (retry_ask_yes_no(&tries,
                            "Unlink of file '%s' failed. "
                            "Should I try again?",
                            pathname));
  return -1;
}
...
int mingw_open(const char *filename, int oflags, ...)
{
   ...
   */
  if (append_atomically < 0 && the_repository && the_repository->commondir && git_config_get_bool("windows.appendatomically", &append_atomically))
    append_atomically = 1;

  if (append_atomically && (oflags & O_APPEND) && !is_local_named_pipe_path(filename))
    ...
    if (GetFileType(h) != FILE_TYPE_PIPE)
    {
      if (orig == EINVAL)
      {
        ...
        DWORD   ret        = GetFinalPathNameByHandleW(h, path,
                                                       ARRAY_SIZE(path), 0);
        UINT    drive_type = ret > 0 && ret < ARRAY_SIZE(path) ? GetDriveTypeW(path) : DRIVE_UNKNOWN;
...
int mingw_lstat(const char *file_name, struct stat *buf)
{
  ...
  if (GetFileAttributesExW(wfilename, GetFileExInfoStandard, &fdata))
  {
    ...
    buf->st_size  = S_ISLNK(buf->st_mode) ? link_len : fdata.nFileSizeLow | (((off_t)fdata.nFileSizeHigh) << 32);
    buf->st_dev = buf->st_rdev = 0; /* not used by Git */
    filetime_to_timespec(&(fdata.ftLastAccessTime), &(buf->st_atim));
    filetime_to_timespec(&(fdata.ftLastWriteTime), &(buf->st_mtim));
    filetime_to_timespec(&(fdata.ftCreationTime), &(buf->st_ctim));
    if (S_ISREG(buf->st_mode) && are_wsl_compatible_mode_bits_enabled())
    {
...
int mingw_stat(const char *file_name, struct stat *buf)
{
  ...
  if (hnd == INVALID_HANDLE_VALUE)
  {
    ...
    if (err == ERROR_ACCESS_DENIED && !mingw_lstat(file_name, buf) && !S_ISLNK(buf->st_mode))
      /*
...
char *mingw_mktemp(char *template)
{
  ...
  if (is_dir_sep(template[0]) && !is_dir_sep(template[1]) && iswalpha(wtemplate[0]) && wtemplate[1] == L':')
  {
...
static char *is_busybox_applet(const char *cmd)
{
  ...
  /* Avoid infinite loop */
  if (!strncasecmp(cmd, "busybox", 7) && (!cmd[7] || !strcasecmp(cmd + 7, ".exe")))
    return NULL;
  ...
  return string_list_has_string(&applets, cmd) ? xstrdup(busybox_path) : NULL;
}
...
int mingw_kill(pid_t pid, int sig)
{
  ...
  {
    HANDLE h = OpenProcess(PROCESS_CREATE_THREAD | PROCESS_QUERY_INFORMATION | PROCESS_VM_OPERATION | PROCESS_VM_WRITE | PROCESS_VM_READ | PROCESS_TERMINATE,
                           FALSE, pid);
    int    ret;
...
int mingw_rename(const char *pold, const char *pnew)
{
  ...
  if (xutftowcs_long_path(wpold, pold) < 0 || xutftowcs_long_path(wpnew, pnew) < 0)
    return -1;
...
repeat:
  ...
  if (attrs == INVALID_FILE_ATTRIBUTES && (attrs = GetFileAttributesW(wpnew)) != INVALID_FILE_ATTRIBUTES)
  {
...
int link(const char *oldpath, const char *newpath)
{
  ...
  if (xutftowcs_long_path(woldpath, oldpath) < 0 || xutftowcs_long_path(wnewpath, newpath) < 0)
    return -1;
                      ...
                      char *tmpbuf, int *plen, DWORD *ptag)
{
  ...
  if ((*plen =
           xwcstoutf(tmpbuf, normalize_ntpath(wbuf), MAX_LONG_PATH))
      < 0)
    return -1;
  return 0;
}
...
int mingw_is_mount_point(struct strbuf *path)
{
  ...
  return (findbuf.dwFileAttributes & FILE_ATTRIBUTE_REPARSE_POINT) && (findbuf.dwReserved0 == IO_REPARSE_TAG_MOUNT_POINT);
}
...
static size_t append_system_bin_dirs(char *path, size_t size)
{
  ...
  if (len == 0 || len >= sizeof(prefix) || !(slash = find_last_dir_sep(prefix)))
    return 0;
  /* strip trailing `git.exe` */
  len = slash - prefix;

  /* strip trailing `cmd` or `mingw64\bin` or `mingw32\bin` or `bin` or `libexec\git-core` */
  if (strip_suffix_mem(prefix, &len, "\\mingw64\\libexec\\git-core") || strip_suffix_mem(prefix, &len, "\\mingw64\\bin"))
  ...
  else if (strip_suffix_mem(prefix, &len, "\\cmd") || strip_suffix_mem(prefix, &len, "\\bin") || strip_suffix_mem(prefix, &len, "\\libexec\\git-core"))
    off += xsnprintf(path + off, size - off,
...
static int is_system32_path(const char *path)
{
  ...
  if (xutftowcs_long_path(wpath, path) < 0 || !GetSystemDirectoryW(system32, ARRAY_SIZE(system32)) || _wcsicmp(system32, wpath))
    return 0;
...
static void setup_windows_environment(void)
{
  ...
  if (!getenv("HOME"))
  {
    ...
    if ((tmp = getenv("HOMEDRIVE")))
    {
      ...
      {
        strbuf_addstr(&buf, tmp);
        if (!is_system32_path(buf.buf) && is_directory(buf.buf))
...
int handle_long_path(wchar_t *path, int len, int max_path, int expand)
{
   ...
   */
  if ((len < 2 || (!is_dir_sep(path[0]) && path[1] != ':')) && (current_directory_len + len < max_path))
    return len;
  ...
  if (result < max_path)
  {
    ...
    if (is_wdir_sep(path[0]) && !is_wdir_sep(buf[0]) && buf[1] == L':' && is_wdir_sep(buf[2]))
      wcscpy(path, buf + 2);
    else
      wcscpy(path, buf);
    return result;
...
int is_inside_windows_container(void)
{
  ...
  inside_container = ERROR_SUCCESS == RegOpenKeyExA(HKEY_LOCAL_MACHINE, key, 0, KEY_READ, &handle);
  RegCloseKey(handle);
...
int file_attr_to_st_mode(DWORD attr, DWORD tag, const char *path)
{
  ...
  if ((attr & FILE_ATTRIBUTE_REPARSE_POINT) && tag == IO_REPARSE_TAG_SYMLINK)
  {
     ...
     */
    if (path && is_inside_windows_container() && readlink(path, buf, sizeof(buf)) > 27 && starts_with(buf, "/ContainerMappedDirectories/"))
      flag = S_IFDIR;
