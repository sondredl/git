...
{
  while (++pos < the_index.cache_nr && !strcmp(the_index.cache[pos]->name, ce->name))
    ; /* skip */
  return pos;
}
    ...
    const char           *arg)
{
  ...
  if (!check_refname_format(new_branch_info->path, 0) && !read_ref(new_branch_info->path, &branch_rev))
                           ...
                           struct branch_info   *new_branch_info)
{
  ...
  if (!opts->switch_branch_doing_nothing_is_ok && !new_branch_info->name && !opts->new_branch && !opts->force_detach)
  ...
  if (!new_branch_info->commit && opts->new_branch)
  {
    ...
    if (!read_ref_full("HEAD", 0, &rev, &flag) && (flag & REF_ISSYMREF) && is_null_oid(&rev))
                         ...
                         const char *const usagestr[])
{
  ...
  {
    if (opts->accept_pathspec && !opts->empty_pathspec_ok && !opts->patch_mode) /* patch mode is special */
