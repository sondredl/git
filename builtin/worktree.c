                        ...
                        const struct add_opts *opts)
{
  ...
  if (!opts->detach && !strbuf_check_branch_ref(&symref, refname) && ref_exists(symref.buf))
  {
    is_branch = 1;
    if (!opts->force)
                                          ...
                                          int         force_new_branch)
{
  ...
  else
  {
    ...
    if (!detach && !strbuf_check_branch_ref(&s, branch) && ref_exists(s.buf))
...
static const char *dwim_branch(const char *path, const char **new_branch)
{
  ...
  branch_exists = !strbuf_check_branch_ref(&ref, branchname) && ref_exists(ref.buf);
  strbuf_release(&ref);
  if (branch_exists)
    return branchname;
...
static int add(int ac, const char **av, const char *prefix)
{
  ...
  if (new_branch_force)
  {
    ...
    if (!opts.force && !strbuf_check_branch_ref(&symref, new_branch) && ref_exists(symref.buf))
      die_if_checked_out(symref.buf, 0);
    strbuf_release(&symref);
  }
