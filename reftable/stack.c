                                ...
                                struct tempfile                  **tab_file_out)
{
  ...
  if (st->config.default_permissions && chmod(get_tempfile_path(tab_file), st->config.default_permissions) < 0)
  {
    err = REFTABLE_IO_ERROR;
    goto done;
  }
...
int reftable_stack_compact_all(struct reftable_stack             *st,
                               struct reftable_log_expiry_config *config)
{
  return stack_compact_range(st, 0, st->merged->stack_len ? st->merged->stack_len - 1 : 0, config);
}
