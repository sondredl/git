...
static void process_curl_messages(void)
{
    ...
    while (curl_message != NULL)
    {
        if (curl_message->msg == CURLMSG_DONE)
        {
            ...
            while (slot != NULL && slot->curl != curl_message->easy_handle)
            {
                slot = slot->next;
            }
            if (slot)
...
static int curl_empty_auth_enabled(void)
{
     ...
     */
    if (http_auth_methods_restricted && (http_auth_methods & ~empty_auth_useless))
    {
        return 1;
    }
    return 0;
...
{
    if ((!http_auth.username || !*http_auth.username) && (!http_auth.credential || !*http_auth.credential))
    {
...
static int redact_sensitive_header(struct strbuf *header, size_t offset)
{
    ...
    if (trace_curl_redact && (skip_iprefix(header->buf + offset, "Authorization:", &sensitive_header) || skip_iprefix(header->buf + offset, "Proxy-Authorization:", &sensitive_header)))
    {
    ...
    else if (trace_curl_redact && skip_iprefix(header->buf + offset, "Cookie:", &sensitive_header))
    {
...
static int match_curl_h2_trace(const char *line, const char **out)
{
     ...
     */
    if (skip_iprefix(line, "h2h3 [", out) || skip_iprefix(line, "h2 [", out))
    {
        return 1;
    }
...
static void redact_sensitive_info_header(struct strbuf *header)
{
    ...
    if (trace_curl_redact && match_curl_h2_trace(header->buf, &sensitive_header))
    {
...
static void curl_dump_data(const char *text, const unsigned char *ptr, size_t size)
{
    size_t        i;
    struct strbuf out   = STRBUF_INIT;
...
static CURL *get_curl_handle(void)
{
    ...
    if (http_ssl_backend && !strcmp("schannel", http_ssl_backend) && !http_schannel_check_revoke)
    {
    ...
    if (http_ssl_backend && !strcmp("schannel", http_ssl_backend) && !http_schannel_use_ssl_cainfo)
    {
    ...
    else if (curl_http_proxy)
    {
        ...
        if (proxy_auth.path)
        {
            ...
            if (strcasecmp(proxy_auth.host, "localhost") != 0)
            {
...
void http_init(struct remote *remote, const char *url, int proactive_auth)
{
    ...
    if (url)
    {
        ...
        if (!ssl_cert_password_required && getenv("GIT_SSL_CERT_PASSWORD_PROTECTED") && starts_with(url, "https://"))
        {
...
int start_active_slot(struct active_request_slot *slot)
{
    ...
    if (curlm_result != CURLM_OK && curlm_result != CURLM_CALL_MULTI_PERFORM)
    {
...
void run_active_slot(struct active_request_slot *slot)
{
    ...
    while (!finished)
    {
        ...
        if (slot->in_use)
        {
             ...
             */
            if (max_fd < 0 && (select_timeout.tv_sec > 0 || select_timeout.tv_usec > 50000))
            {
                select_timeout.tv_sec  = 0;
                select_timeout.tv_usec = 50000;
            }
                        ...
                        const struct http_get_options *options)
{
    ...
    if (options && options->initial_request && http_follow_config == HTTP_FOLLOW_INITIAL)
    {
        curl_easy_setopt(slot->curl, CURLOPT_FOLLOWLOCATION, 1);
    }
...
int http_get_info_packs(const char *base_url, struct packed_git **packs_head)
{
    ...
    while (*data)
    {
        if (skip_prefix(data, "P pack-", &data) && !parse_oid_hex(data, &oid, &data) && skip_prefix(data, ".pack", &data) && (*data == '\n' || *data == '\0'))
        {
...
static const char *default_index_pack_args[] = { "index-pack", "--stdin", NULL };
...
int finish_http_pack_request(struct http_pack_request *preq)
{
    ...
    ip.git_cmd = 1;
    ip.in      = tmpfile_fd;
    strvec_pushv(&ip.args, preq->index_pack_args ? preq->index_pack_args : default_index_pack_args);
                                                    ...
                                                    const struct object_id *oid)
{
     ...
     * fetched.
     */
    prevlocal = open(prevfile.buf, O_RDONLY | O_CLOEXEC);
    if (prevlocal != -1)
    {
        do
        {
