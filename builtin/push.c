                                  ...
                                  struct remote *remote, struct ref *matched)
{
    ...
    if (push_default == PUSH_DEFAULT_UPSTREAM && skip_prefix(matched->name, "refs/heads/", &branch_name))
    {
...
static void setup_default_push_refspecs(int *flags, struct remote *remote)
{
    ...
    switch (push_default)
    {
        ...
        case PUSH_DEFAULT_SIMPLE:
            ...
            if (strcmp(branch->refname, get_upstream_ref(*flags, branch, remote->name)) != 0)
            {
                           ...
                           const struct config_context *ctx, void *cb)
{
    ...
    {
        int val            = git_config_bool(k, v) ? RECURSE_SUBMODULES_ON_DEMAND : RECURSE_SUBMODULES_OFF;
