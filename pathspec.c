                                    ...
                                    const char           *elem)
{
    ...
    for (pos = elem + 2; *pos && *pos != ')'; pos = nextat)
    {
        ...
        {
            if (strlen(pathspec_magic[i].name) == len && !strncmp(pathspec_magic[i].name, pos, len))
            {
                         ...
                         const char *file, int nul_term_line)
{
    ...
    strbuf_getline_fn getline_fn  = nul_term_line ? strbuf_getline_nul : strbuf_getline;
    ...
    for (i = 0; i < pathspec->nr; i++)
    {
        ...
        if (item.nowildcard_len < item.len)
        {
            ...
            for (pos = 0; pos < istate->cache_nr; pos++)
            {
                 ...
                 */
                if (item.nowildcard_len > ce_namelen(ce) && !strncmp(item.original, ce->name, ce_namelen(ce)))
                {
                    res = 1;
                    break;
                }
                 ...
                 */
                if (!strncmp(item.original, ce->name, item.nowildcard_len) && wildmatch(item.original, ce->name, 0))
                {
                    res = 1;
                    break;
                }
