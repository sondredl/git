...
int clar_summary_shutdown(struct clar_summary *summary)
{
    ...
    while (report != NULL)
    {
        ...
        if (last_suite == NULL || strcmp(last_suite, report->suite) != 0)
        {
            if (clar_summary_testsuite(summary, 0, report->suite,
                                       report->start, _clar.tests_ran, _clar.total_errors, 0)
                < 0)
                goto on_error;
        }
        ...
        while (error != NULL)
        {
            if (clar_summary_failure(summary, "assert",
                                     error->error_msg, error->description)
                < 0)
                goto on_error;
    ...
    if (clar_summary_close_tag(summary, "testsuites", 0) < 0 || fclose(summary->fp) != 0)
        goto on_error;
