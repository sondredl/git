...
static void sort_snapshot(struct snapshot *snapshot)
{
  ...
  while (pos < eof)
  {
    ...
    if (sorted && nr > 1 && cmp_packed_ref_records(&records[nr - 2], &records[nr - 1]) >= 0)
      sorted = 0;
...
static int next_record(struct packed_ref_iterator *iter)
{
  ...
  if (iter->eof - p < the_hash_algo->hexsz + 2 || parse_oid_hex(p, &iter->oid, &p) || !isspace(*p++))
  ...
  {
    p = iter->pos + 1;
    if (iter->eof - p < the_hash_algo->hexsz + 1 || parse_oid_hex(p, &iter->peeled, &p) || *p++ != '\n')
      die_invalid_line(iter->snapshot->refs->path,
                       iter->pos, iter->eof - iter->pos);
    iter->pos = p;
