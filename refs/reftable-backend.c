...
static int reftable_ref_iterator_advance(struct ref_iterator *ref_iterator)
{
  ...
  while (!iter->err)
  {
     ...
     */
    if (!starts_with(iter->ref.refname, "refs/") && !(iter->flags & DO_FOR_EACH_INCLUDE_ROOT_REFS && (is_pseudoref(&iter->refs->base, iter->ref.refname) || is_headref(&iter->refs->base, iter->ref.refname))))
    {
      continue;
    }

    if (iter->prefix_len && strncmp(iter->prefix, iter->ref.refname, iter->prefix_len))
    {
      iter->err = 1;
      break;
    }
...
static int write_transaction_table(struct reftable_writer *writer, void *cb_data)
{
  ...
  for (i = 0; i < arg->updates_nr; i++)
  {
    ...
    else if (u->flags & REF_HAVE_NEW && (u->flags & REF_FORCE_CREATE_REFLOG || should_write_log(&arg->refs->base, u->refname)))
    {
...
static int write_create_symref_table(struct reftable_writer *writer, void *cb_data)
{
   ...
   * never happens.
   */
  if (!create->logmsg || !refs_resolve_ref_unsafe(&create->refs->base, create->target, RESOLVE_REF_READING, &new_oid, NULL) || !should_write_log(&create->refs->base, create->refname))
    return 0;
