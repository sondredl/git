...
static void cmp_two_packs(struct pack_list *p1, struct pack_list *p2)
{
    ...
    size_t               p1_step;
    size_t               p2_step;
    const unsigned char *p1_base;
    const unsigned char *p2_base;
    struct llist_item   *p1_hint = NULL;
    ...
    while (p1_off < p1->pack->num_objects * p1_step && p2_off < p2->pack->num_objects * p2_step)
    {
        ...
        if (cmp < 0)
        { /* p1 has the object, p2 doesn't */
            p1_off += p1_step;
        }
        else
        { /* p2 has the object, p1 doesn't */
            p2_off += p2_step;
        }
    }
}
...
static size_t sizeof_union(struct packed_git *p1, struct packed_git *p2)
{
    ...
    while (p1_off < p1->num_objects * p1_step && p2_off < p2->num_objects * p2_step)
    {
...
{
    struct pack_list *pl;
    struct pack_list *unique     = NULL;
    struct pack_list *non_unique = NULL;
    struct llist     *missing;
    struct llist     *unique_pack_objects;
...
{
    struct pack_list *local;
    struct pack_list *alt;
