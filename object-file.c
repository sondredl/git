...
int mkdir_in_gitdir(const char *path)
{
    if (mkdir(path, 0777))
    {
         ...
         */
        if (lstat(path, &st) || !S_ISLNK(st.st_mode) || strbuf_readlink(&sb, path, st.st_size) || !is_absolute_path(sb.buf) || mkdir(sb.buf, 0777))
        {
            strbuf_release(&sb);
            errno = saved_errno;
            return -1;
...
void restore_primary_odb(struct object_directory *restore_odb, const char *old_path)
{
    ...
    if (strcmp(old_path, cur_odb->path) != 0)
    {
...
{
    return check_and_freshen_local(oid, freshen) || check_and_freshen_nonlocal(oid, freshen);
}
                                       ...
                                       struct object_info *oi, unsigned flags)
{
    ...
    if (oi->whence == OI_PACKED)
    {
        ...
        oi->u.packed.is_delta = (rtype == OBJ_REF_DELTA || rtype == OBJ_OFS_DELTA);
    }
                                   ...
                                   struct object_info *input_oi, unsigned flags)
{
    ...
    /* Is new_oi needed? */
    oi = input_oi;
    if (input_oi && (input_oi->delta_base_oid || input_oi->sizep || input_oi->contentp))
    {
        new_oi = *input_oi;
                        ...
                        struct object_id *oid)
{
    ...
    if (repo_has_object_file_with_flags(the_repository, oid, OBJECT_INFO_QUICK | OBJECT_INFO_SKIP_FETCH_OBJECT) || find_cached_object(oid))
    {
        return 0;
    }
                                 ...
                                 struct object_id       *actual_oid_return)
{
    ...
    while (1)
    {
        ...
        if (ref_length + the_hash_algo->hexsz > isize || memcmp(buffer, ref_type, ref_length) != 0 || get_oid_hex((char *)buffer + ref_length, &actual_oid))
        {
            free(buffer);
            return NULL;
        }
                               ...
                               enum finalize_object_file_flags flags)
{
    ...
    if (ret)
    {
        ...
        if (!(flags & FOF_SKIP_COLLISION_CHECK) && check_collision(tmpfile, filename))
            return -1;
        unlink_or_warn(tmpfile);
    }
...
{
    int fd;
    int dirlen = directory_size(filename);
                              ...
                              time_t mtime, unsigned flags)
{
    ...
    if (mtime)
    {
        ...
        utb.actime  = mtime;
        utb.modtime = mtime;
        if (utime(tmp_file.buf, &utb) < 0 && !(flags & HASH_SILENT))
        {
               ...
               unsigned flags)
{
    ...
    unsigned object_info_flags = OBJECT_INFO_SKIP_FETCH_OBJECT | (quick ? OBJECT_INFO_QUICK : 0);
             ...
             enum object_type type, const char *path, unsigned flags)
{
    ...
    else if (st->st_size <= big_file_threshold || type != OBJ_BLOB || (path && would_convert_to_git(istate, path)))
    {
                                ...
                                void                *data)
{
    ...
    while ((de = readdir_skip_dot_and_dotdot(dir)))
    {
        ...
        if (namelen == the_hash_algo->hexsz - 2 && !hex_to_bytes(oid.hash + 1, de->d_name, the_hash_algo->rawsz - 1))
        {
                                ...
                                const struct object_id  *oid)
{
    ...
    if (subdir_nr < 0 || subdir_nr >= bitsizeof(odb->loose_objects_subdir_seen))
    {
                            ...
                            const struct object_id *expected_oid)
{
     ...
     */
    while (total_read <= size && (status == Z_OK || (status == Z_BUF_ERROR && !stream->avail_out)))
    {
        stream->next_out  = buf;
